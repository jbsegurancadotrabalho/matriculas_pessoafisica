...
public interface MatriculasRepository  extends JpaRepository<Matriculas, UUID> {
	...
	List<Matriculas> findAllMatriculas();
	 ...
	 @Query("SELECT m FROM Matriculas m WHERE MONTH(m.dataHoraCriacao) = :mes AND m.pessoafisica IS NOT NULL")
	    List<Matriculas> findMatriculasPessoaFisicaByMes(@Param("mes") int mes);
